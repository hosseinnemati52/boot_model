...
def distributions_plotter():
    ...
    for time_Win_C in range(N_time_windows):
        WT_enter_fit_mean.append( np.mean(WT_enter_fit_all_runs[time_Win_C], axix=0) )
        WT_exit_fit_mean.append( np.mean(WT_exit_fit_all_runs[time_Win_C], axix=0) )
    ...
    for time_Win_C in range(N_time_windows):
        ...
        plt.bar(bin_centers, WT_enter_fit_mean, width=np.diff(bin_edges), align='center', alpha=0.7, color='blue')
        plt.xlabel("fitness")
        plt.ylabel("PDF")
    ...
    for time_Win_C in range(N_time_windows):
        ...
        # Plotting the histogram
        plt.bar(bin_centers, WT_exit_fit_mean, width=np.diff(bin_edges), align='center', alpha=0.7, color='red')
        plt.xlabel("fitness")
        plt.ylabel("PDF")
...
WT_enter_fit_all_runs = N_time_windows * [np.zeros((N_runs, len(fit_hist_bins)-1))]
WT_exit_fit_all_runs = N_time_windows * [np.zeros((N_runs, len(fit_hist_bins)-1))]
...
for runC in range(N_runs):
    ...
    ## distributions ##
    for time_Win_C in range(N_time_windows):
        WT_enter_fit_all_runs[time_Win_C][runC,:] = np.loadtxt(folderName+"/pp_data/enter_fit_plot_"+str(time_Win_C)+".txt", delimiter=',')
        WT_exit_fit_all_runs[time_Win_C][runC,:] = np.loadtxt(folderName+"/pp_data/exit_fit_plot_"+str(time_Win_C)+".txt", delimiter=',')
    ## distributions ##
