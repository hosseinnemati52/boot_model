...
def landscape_plotter(t, snapshotInd):
    ...
    for i in range(NCells):
        ...
        if cellState[i]==DIFF_STATE or cellState[i]==G0_STATE:
            ...
            # ax1.add_patch(polgon)
            plt.scatter(cellPhi[i], cellFitness[i,0], alpha=0.8, color = color, marker = "p")
        else:
            # circle = patches.Circle((cellPhi[i], cellFitness[i,0]), radius=normalize_factor*cellR[i], edgecolor='k', facecolor=color, alpha=0.8)
            # ax1.add_patch(circle)
            plt.scatter(cellPhi[i], cellFitness[i,0], alpha=0.8, color = color, marker = "o")
    # plt.scatter(x_sph, y_sph, s=size_vec, c=cell_sph, alpha=1, cmap='viridis')
    # ax1.axis("equal")
    # plt.xlim((0,Lx))
    # plt.ylim((0,Ly))
    ...
    plt.plot([x_max, x_max], [variables['Fit_Th_G1_arr'], y_max], linestyle='dashed', color='k')
    plt.plot([x_min, x_max], [variables['Fit_Th_G1_arr'],variables['Fit_Th_G1_arr']], linestyle='dashed', color='k')
    ...
    plt.plot([x_min, x_G1_Border], [variables['Fit_Th_Diff'],variables['Fit_Th_Diff']], linestyle='dashed', color='k')
